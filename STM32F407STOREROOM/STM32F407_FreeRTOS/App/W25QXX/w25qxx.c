...
{ 
    ...
    if(W25QXX_TYPE==W25Q256)                //SPI FLASH为W25Q256
    {
		...
		{
			SPI_CS_L; 			        //选中
			SPI1_WriteByte(W25X_Enable4ByteAddr);//发送进入4字节地址模式指令   
			SPI_CS_H;       		        //取消片选   
		}
    }
}  
...
{   
	...
	SPI_CS_L;                            //使能器件   
	SPI1_WriteByte(command);            //发送写取状态寄存器命令    
	SPI1_WriteByte(sr);                 //写入一个字节  
	SPI_CS_H;                            //取消片选     	      
}   
//W25QXX写使能	
//将WEL置位   
...
{ 
    ...
    {
        SPI1_WriteByte((uint8_t)((ReadAddr)>>24));    
    }
...
void W25QXX_Write_Page(uint8_t* pBuffer,uint32_t WriteAddr,uint16_t NumByteToWrite)
{
    ...
    if(W25QXX_TYPE==W25Q256)                //如果是W25Q256的话地址为4字节的，要发送最高8位
    {
        SPI1_WriteByte((uint8_t)((WriteAddr)>>24)); 
    }
...
{  
    ...
    W25QXX_Wait_Busy();   
  	SPI_CS_L;                            //使能器件   
    SPI1_WriteByte(W25X_SectorErase);   //发送扇区擦除指令 
    if(W25QXX_TYPE==W25Q256)                //如果是W25Q256的话地址为4字节的，要发送最高8位
    {
        SPI1_WriteByte((uint8_t)((Dst_Addr)>>24)); 
    }
    SPI1_WriteByte((uint8_t)((Dst_Addr)>>16));  //发送24bit地址    
...
{ 
  	SPI_CS_L;                            //使能器件   
    SPI1_WriteByte(W25X_PowerDown);     //发送掉电命令  
	SPI_CS_H;                            //取消片选     	      
    delay_us(3);                            //等待TPD  
}   
//唤醒
...
{  
  	SPI_CS_L;                                //使能器件   
    SPI1_WriteByte(W25X_ReleasePowerDown);  //  send W25X_PowerDown command 0xAB    
	SPI_CS_H;                                //取消片选     	      
    delay_us(3);                                //等待TRES1
}   