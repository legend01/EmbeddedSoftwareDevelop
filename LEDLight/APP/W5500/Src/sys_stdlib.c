...
/*
	...
	@retval:若参数s1和s2字符串相同，则返回0；
			...
			若s1小于s2，则返回小于0的值。
*/
 int sys_strncasecmp (const char *s1, const char *s2,  int n)
{
    char c1, c2;
     int k = 0;
     int result = 0;
...
*/
char * sys_strncat (char *dest, const char *src,  int n)
{
    char *dp;
    char *sp = (char *)src;
...
/*
	...
	@Notice:src和dest所指内存区域不可以重叠且dest必须有
			足够的空间来容纳src的字符长度+'\0'
*/
char * sys_strncpy (char *dest, const char *src,  int n)
{
    char *dp = (char *)dest;
    char *sp = (char *)src;
...
*/
char *sys_strrchr(char *s, char c)
{
	char *p_char = NULL;
	if(s == NULL)
    {
...
/*
	...
	@Notice:
*/
void * sys_memset (void *s,  int c, int n)
{
...
/********************************************************************/
static  int printk_mknumstr (unsigned char *numstr, void *nump,  int neg,  int radix)
{
    int a, b, c;
     int ua, ub, uc;
